...
from model import RASC
from utils import get_logger, sparse_to_tuple
...
def train(model_args, data):
    ...
    # define the model
    model = RASC(args, n_relations, neighbor_params, path_params)
    ...
    for step in range(args.epoch):
        ...
        # evaluation
        logger.info('epoch{:2d}' .format(step))
        ...
        logger.info('train acc: %.3f   valid acc: %.3f   test acc: %.3f' % (train_acc, valid_acc, test_acc))
        print('train acc: %.3f   valid acc: %.3f   test acc: %.3f' % (train_acc, valid_acc, test_acc))
        mrr, mr, hit1, hit3, hit5, hit10 = calculate_ranking_metrics(test_triplets, test_scores, true_relations)
        current_res += '   mrr: %.3f   mr: %.3f   h1: %.3f   h3: %.3f   h5: %.3f  h10: %.3f' % (mrr, mr, hit1, hit3, hit5, hit10)
        logger.info('mrr: %.3f   mr: %.3f   h1: %.3f   h3: %.3f   h5: %.3f  h10: %.3f' % (mrr, mr, hit1, hit3, hit5, hit10))
        print('           mrr: %.3f   mr: %.3f   h1: %.3f   h3: %.3f   h5: %.3f  h10: %.3f' % (mrr, mr, hit1, hit3, hit5, hit10))
        print()
