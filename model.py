...
class RASC(nn.Module):
    ...
    def _get_neighbors_and_masks(self, relations, entity_pairs, train_edges):
        ...
        for i in range(self.context_hops):
            if i == 0:
                ...
                neighbor_entities = torch.index_select(self.edge2entities, 0, edges_list[-1].view(-1)).view([self.batch_size, -1])  # 找到所有邻边的实体
            neighbor_edges = torch.index_select(self.entity2edges, 0, neighbor_entities.view(-1)).view([self.batch_size, -1]) # 找到所有实体的邻边
    ...
    def _aggregate_neighbors(self, edge_list, mask_list):
        ...
        edge_vectors = [torch.index_select(self.relation_features, 0, edge_list[0])]    # 为128个批次中的每一条初始边分配向量
        ...
        for i in range(self.context_hops):
            ...
            for hop in range(self.context_hops - i):
                vector = aggregator(relation_label=edge_vectors[0], self_vectors=edge_vectors[hop], neighbor_vectors=edge_vectors[hop + 1].view(neighbors_shape), masks=mask_list[hop].view(masks_shape))
    ...
    def _aggregate_paths(self, inputs):
        ...
        if self.path_agg == 'mean':
            ...
            aggregated_neighbors = self.aggregated_neighbors.unsqueeze(1)  # [batch_size, 1, n_relations]  在第1个维度添加一个维度
